...
function Tip({ active, payload, unitLabel }: any) {
  ...
  const d = payload[0]; const total = d.payload.total || 1;
  return (
    <div className="rounded-xl border px-4 py-3 text-xs"
      ...
      <div style={{ color: 'var(--text3)' }}>{((d.value / total) * 100).toFixed(1)}% of total</div>
    </div>
  );
}
    ...
    <div className="glass h-full fade-up flex flex-col gap-3 p-5" style={{ animationDelay: '280ms' }}>
      ...
      <div className="relative">
        <ResponsiveContainer width="100%" height={180}>
          <PieChart>
            <Pie data={rich} cx="50%" cy="50%" innerRadius={58} outerRadius={90}
              dataKey="value" paddingAngle={2} strokeWidth={0}>
              {rich.map((d, i) => <Cell key={i} fill={d.fill} />)}
            </Pie>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          <div className="text-[11px]" style={{ color: 'var(--text3)' }}>Total</div>
          <div className="font-display font-bold text-base" style={{ color: 'var(--accent)' }}>{fmt(total, 0)}</div>
          <div className="text-[11px]" style={{ color: 'var(--text3)' }}>tCO₂</div>
        </div>
      </div>

      {/* Legend */}
      <div className="flex flex-col gap-1.5 overflow-y-auto pr-1" style={{ maxHeight: 160 }}>
        {rich.map((d, i) => (
